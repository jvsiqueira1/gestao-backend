    ...
    html: `
      <div style="font-family: 'Segoe UI', Arial, sans-serif; background: #f8fafc; padding: 32px; color: #222;">
        <div style="max-width: 480px; margin: 0 auto; background: #fff; border-radius: 12px; box-shadow: 0 2px 8px #0001; padding: 32px;">
          ...
          <a href="${process.env.FRONTEND_URL}/perfil" style="display: inline-block; background: #0e7490; color: #fff; text-decoration: none; padding: 12px 28px; border-radius: 8px; font-weight: 600; font-size: 1.1em; margin-bottom: 16px;">Regularizar Assinatura</a>
    ...
    try {
      switch (event.type) {
        case 'checkout.session.completed': {
          ...
          if (session.mode === 'subscription') {
            if (!session.metadata || !session.metadata.user_id) {
              ...
              return res
                .status(400)
                .json({ error: 'user_id ausente em session.metadata' });
            }
          ...
          if (user) {
            ...
            if (subscription.status === 'active') {
              ...
              if (subscription.current_period_end) {
                updateData.premium_until = new Date(
              ...
              if (subscription.items && subscription.items.data.length > 0) {
                ...
                if (item.price && item.price.unit_amount) {
                  updateData.plan =
                    item.price.unit_amount === 15000 ? 'anual' : 'mensal';
                }
              }
            }
        ...
        case 'invoice.payment_failed': {
          ...
          const _failResult = await prisma3.user.updateMany({
        ...
        case 'customer.subscription.trial_will_end': {
          ...
          const user = await prisma4.user.findFirst({
            where: { stripe_customer_id: subscription.customer }
          });
          if (user) {
